...
| **5** | **Command & Example Correctness** | Are code examples, shell snippets, and CLI flags copy-pasteable and matching real manifests? Do samples meet hygiene (language tag, ≤80 cols, ≤25 lines split with prose, block explained, no `<TOKEN>` placeholders)? | Test commands against repository tools or verify against manifests. |
