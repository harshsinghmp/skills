...
Round N (N = 1..max_rounds):
  ...
  2. AUDIT (Blind A/B): Spawn isolated Fresh Critic subagent with NO memory of builder reasoning.
      ...
      - **Metric honesty** (source: `addyosmani/agent-skills` agents `web-performance-auditor`): never fabricate a measurement — unmeasured = `not measured`, lab ≠ field.
      - **No-persona-spawns** (source: `addyosmani/agent-skills` agents rule): the critic never delegates to sub-reviewers; orchestration belongs to the loop caller.
      - **Prove-It** (H12 test-engineer rule): already landed as the TDD iron law above (S4) — cited, not duplicated.
      - **Adversarial reviewer** (source: lane H1 #7 `doubt-driven-development`): prompt critic to disprove, not confirm; evidence = ARTIFACT+CONTRACT only, never CLAIM; non-triviality gate (5 criteria); CLAIM→EXTRACT→DOUBT→RECONCILE→STOP with 3-cycle cap, then STOP/escalate.
  ...
  3. GATE: Run automated proof suite AND integrity checks AND Web App Security & Visual Gate:
      ...
      - **Discover-the-Stack-First preamble** (source: `addyosmani/agent-skills` `test-driven-development`): read package.json/wrappers/framework/conventions/CI before the first test; focused-test command in the loop, full suite before completion; never assume `npm test`.
      - **Testing-patterns bar** (source: `addyosmani/agent-skills` `references/testing-patterns`): AAA structure, names `[unit]-[behavior]-[condition]`, mock-at-boundaries-only matrix (mock DB/HTTP/FS/clock; never mock internals or pure functions), and the 8-row anti-pattern table. JS/TS syntax illustrative — principles portable across stacks.
